...
async function fetchAndStoreStockData() {
    ...
    try {
        ...
        const historyCollection = db.collection("stock_history");

        // Preserve existing data in history collection
        const existingStocks = await stockCollection.find({}).toArray();
        if (existingStocks.length > 0) {
            const historyData = existingStocks.map(stock => {
                const { _id, ...stockWithoutId } = stock; // Exclude _id
                return {
                    ...stockWithoutId,
                    historical_date: stock.fetched_at,
                    archived_at: new Date().toISOString()
                };
            });
            await historyCollection.insertMany(historyData);
            console.log("Preserved historical data in stock_history");
        }
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                // Update stocks collection with new data
